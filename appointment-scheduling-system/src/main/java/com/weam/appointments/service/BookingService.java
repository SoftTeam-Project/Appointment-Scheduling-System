...
public class BookingService {
    ...
    public boolean bookAppointment(int slotId, String username, int durationMinutes, int participants) {
        ...
        Appointment appointment = new Appointment(
                0, slotId, username, slot.date, slot.time,
                durationMinutes, participants, "Confirmed"
        );
    ...
    public boolean cancelAppointment(int appointmentId, String username) {
        ...
        if (!app.getUsername().equals(username)) return false;

        LocalDateTime appointmentDateTime = LocalDateTime.parse(app.getDate() + "T" + app.getTime());
        if (appointmentDateTime.isBefore(LocalDateTime.now())) return false;
        ...
        try (Connection con = Db.getConnection()) {
            ...
            con.setAutoCommit(false);
            if (!deleteAppointmentWithConnection(appointmentId, con)) throw new SQLException("Delete failed");
            if (!decrementBookedCountWithConnection(slotId, con)) throw new SQLException("Decrement failed");
            con.commit();
            return true;
    ...
    private boolean saveAppointmentWithConnection(Appointment app, Connection con) throws SQLException {
        ...
        String sql = "INSERT INTO appointments(slot_id, username, appointment_date, appointment_time, " +
                     "duration_minutes, participants, status) VALUES (?,?,?,?,?,?,?)";
